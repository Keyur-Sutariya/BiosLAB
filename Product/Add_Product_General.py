...
current_row = 3
Product_name_row = 3
Product_content_row = 3
Product_short_name_row = 3
i = 1
j = 7

driver = webdriver.Firefox()
File = openpyxl.load_workbook("/home/tipl-pc-21/Downloads/ProductG.xlsx")
...
while current_row < last_Row:
    i = i + 1
    Product_name_row_stored = Active_File.cell(row=Product_name_row, column=2)
    Product_content_row_stored = Active_File.cell(row=Product_content_row, column=3)
    Product_short_name_row_stored = Active_File.cell(row=Product_short_name_row, column=5)
    ...
    Product_short_name_row_stored_value = Product_short_name_row_stored.value
    ...
    time.sleep(1)
    driver.find_element(By.XPATH, "//input[@name='Code']").send_keys(j, "397", i)
    time.sleep(1)
    action.send_keys(Keys.TAB).perform()
    driver.find_element(By.XPATH, " //input[@name='ShortName']").send_keys(" ", Product_short_name_row_stored_value)
    ...
    driver.find_element(By.XPATH, "//input[@name='CodeRef1']").send_keys("Reference Gen", i, j)

    driver.find_element(By.XPATH, "//input[@name='OnHold']").click()
    time.sleep(1)
    driver.find_element(By.XPATH, "//input[@name='HsnCode']").send_keys(j, "428", i)
    time.sleep(1)
    action.send_keys(Keys.TAB).perform()
    action.send_keys(Keys.DOWN).perform()
    action.send_keys(Keys.ENTER).perform()
